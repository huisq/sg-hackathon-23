...
*/
    ...
    public entry fun archive_link(
        ...
        let state = borrow_global_mut<State>(@admin);
        let previous_archieve = string::utf8(b"");
        if(!simple_map::contains_key(&state.websites, &site_url)){
            ...
            let new_archieve = Archieve {
                hash: site_ipfs_hash,
                timestamp: timestamp::now_seconds()
            };
            simple_map::add(&mut state.websites, site_url, signer::address_of(&obj_signer));
            move_to<Archieve>(&obj_signer, new_archieve);
        }else{
            let archieve = borrow_global_mut<Archieve>(*simple_map::borrow(&state.websites, &site_url));
            previous_archieve = archieve.hash;
            archieve.hash = site_ipfs_hash;
            archieve.timestamp = timestamp::now_seconds();
        };
        ...
        event::emit_event<ArchiveLinkEvent>(
            ...
            ArchiveLinkEvent {
                ...
                previous_ipfs: previous_archieve,
    ...
    fun test_delete_review_success(
        ...
        operator: &signer,
        reviewer: &signer
    ) acquires State, ReviewToken, Archieve {
        let admin_address = signer::address_of(admin);
        let reviewer_address = signer::address_of(reviewer);
        let operator_address = signer::address_of(operator);
        account::create_account_for_test(admin_address);
    ...
    fun test_archieve(
        ...
        let archieve = borrow_global<Archieve>(*simple_map::borrow(&state.websites, &site_url));
        assert!(archieve.hash == site_ipfs_hash, 4);
